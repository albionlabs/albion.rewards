...
async function generateRandomBlocks(startBlock: number, endBlock: number, startTimestamp: number, endTimestamp: number, tokenAddress: string) {
    ...
    // Create output directory and write to file using timestamp-based naming
    ...
    const outputDir = `output/${startDateStr}_to_${endDateStr}/${tokenAddress}`;
...
async function main() {
    ...
    if (args.length < 3) {
        console.error("Usage: npm run generate-snapshots <startTimestamp> <endTimestamp> <tokenAddress>");
        console.error("Example: npm run generate-snapshots 1725148800 1727740799 0xd5316ca888491575befc0273a00de2186c53f760");
        console.error("Timestamps should be Unix timestamps in seconds");
        process.exit(1);
    }
    ...
    if (isNaN(startTimestamp) || isNaN(endTimestamp)) {
        console.error("Invalid timestamps. Both start and end timestamps must be valid Unix timestamps in seconds");
        process.exit(1);
    }
    
    if (startTimestamp >= endTimestamp) {
        console.error("Invalid timestamp range. Start timestamp must be less than end timestamp");
        process.exit(1);
    }
    ...
    console.log(`Generating snapshots from ${startDate.toISOString()} to ${endDate.toISOString()} for token ${tokenAddress}...`);
    await getBlocksForTimestampRange(startTimestamp, endTimestamp, tokenAddress);
}
